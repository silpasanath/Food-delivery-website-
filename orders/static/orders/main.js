...
function add_to_cart(info){
  ...
  if (cart_retrieved === null || cart_retrieved === ''){
    //make a new cart
    var cart = [info];
    localStorage.setItem('cart', JSON.stringify(cart));
    location.reload()
  }else{
    console.log(cart_retrieved==='')
...
function display_notif(type, info="No info provided"){
  ...
  switch (type){
    case "add to cart":
      toastr.success(info.item_description + ': &#8377;' + info.price + ' '+ info.quantity + ' nos', 'Added to Cart');
      break;
...
//     console.log("TOPping choices are "+topping_choices)
...
function pizza_toppings(id, prize_of_topping,prize_of_pizza){
  console.log();
  const pizza = prize_of_pizza
  const tprice = document.getElementById(`price${id}`);
  let price =tprice.querySelector("strong").textContent 
  let quantity =document.querySelector(`quantity${id}`).value
  const topping = prize_of_topping.split('--')[1]
  if(topping == undefined){
    tprice.querySelector("strong").textContent = price
  }else{
    price = parseint(prize_of_topping)* parseInt(quantity)
    tprice.querySelector("strong").textContent = parseFloat(pizza) + parseFloat(topping)
  }
}
